...
{
    // For native package, all classes are already loaded so it's no
    // problem to fully enumerate during prepass.
    ...
    if (Package == nullptr)
    {
        UE_LOG(LogUnrealisticDocs, Warning,
               TEXT("Failed to find specified package '%s', skipping."),
               *PkgName);
        return;
    }

    // Make sure it's fully loaded (probably unnecessary since only native
    // packages here, but no harm)
    Package->FullyLoad();

    // TODO(artsiom.drapun): Work out why the below enumeration is called
    // twice for every class in the package (it's called on the exact same
    // uclass instance)
    ...
    {
        // The BP action database appears to be keyed either on native UClass
        // objects, or, in the case of blueprints, on the Blueprint object
        // itself, as opposed to the generated class.

        UObject* ObjectToProcess = nullptr;

        // Native class or interface
        if (auto Class = Cast<UClass>(Obj))
        {
            if (Class->HasAllClassFlags(CLASS_Native) && !Class->HasAnyFlags(
        ...
        {
            UE_LOG(LogUnrealisticDocs, Log,
                   TEXT("Enumerating object '%s' in package '%s'"),
                   *ObjectToProcess->GetName(),
                   *PkgName);
...
{
    return static_cast<float>(CurIndex / (ObjectList.Num() - 1));
}
